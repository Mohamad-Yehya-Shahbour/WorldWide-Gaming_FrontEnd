...
function UserProfile(props) {
  ...
  const drawer = (
    <div>
      ...
      <Divider />
      <List key={"userprofile"}>
        {data.map((item,i)=>(
          <>
            <ListItem key={`userProfile${item.path}`} sx={{marginBottom:"9px"}} button onClick={()=>{
      ...
      >
        ...
        <Container className="mb-3">
                <Row>
                    ...
                    <Col sx={6} className="d-flex justify-content-end">
                      <Button id="add-friend-button" sx={{ color:"white"}}  variant="contained" className="float-right">create post</Button>
                    </Col>
                </Row>
          </Container>
        <UserProfileItem/>
